...
const Modal = ({ title, children, onClose, side }: IModalProps) => {
  ...
  return (
    <motion.div
      ...
      className={`${positionClass} absolute w-fit h-fit bg-[#0d1220]/50 hover:bg-[#181d2c]/50 backdrop-blur-sm border border-white/5 hover:border-white/10 transition-colors duration-500 ease-in-out shadow-md rounded-lg z-50 p-4`}
    >
      <div className="flex flex-1 items-center justify-between drag-handle cursor-grab active:cursor-grabbing">
